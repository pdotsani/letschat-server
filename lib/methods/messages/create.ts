...
export const createMessage = async (res: Response, params: CreateMessageParams | CreateMessageParams[]) => {
  ...
  try {
    const { error } = await client.from('messages').insert(params.map(({ chatId, content }) => ({
      user_id: user.id,
      chat_id: chatId,
      message: content,
      created_at: new Date()
    ...
    return res.json({ message: 'Array of messages created' });
  } catch (error) {
    return res.status(500).json({ error: 'Failed to create message' });
  }  
}